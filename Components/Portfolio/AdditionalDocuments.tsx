...
const Popup: React.FC<PopupProps> = ({
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const uploadPromises = await params.map(async (param, index) => {
        //@ts-ignore
        const imageRes = await uploadImage(param.uploadUrl.uploadUrl, selectedFiles[index]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      ...
      >
        ...
        <p className="text-base font-normal text-[#87878D]">
          To verify your ownership of the claimed air rights and facilitate your passive income, we require additional
          ...
          {selectedFiles?.length > 0 &&
            selectedFiles.map((selectedFile, index) => (
              <div className="flex items-center justify-between" key={index}>
                <div className="w-[40%] truncate sm:w-auto">{selectedFile.name}</div>
                <div className="flex w-[40%] justify-end sm:w-auto">
                  <button onClick={() => removeFile(selectedFile)} className="m-0 text-red-400 sm:mr-3">
                    Remove
                  </button>
                </div>
              </div>
